...
$(document).ready(function(){
 ...
 @if(old('user_id') and  old('group_id'))
	$.ajax({
		...
		type:'post',
		dataType:'html',
		data:{group_id:'{{ old('group_id') }}',_token:'{{ csrf_token() }}',select:'{{ old('user_id') }}'},
		success: function(data)
		{
			$('.user_id').removeClass('hidden');
			$('.user_id').html(data);
 ...
 $.ajax({
 		...
 		dataType:'html',
 		type:'post',
 		data:{_token:'{{ csrf_token() }}',day:'{{ old('in_day') }}',period:'{{ old('period') }}',select:'{{ old('in_time') }}'},
 		beforeSend: function()
 		{
 			$('.in_time_load').removeClass('hidden');
 			$('.in_time').html('');
		...
		<div class="widget-extra body-req portlet light bordered">
			...
			<div class="portlet-body form">
				<div class="col-md-12">
					...
					{!! Form::open(['url'=>aurl('/appointments'),'id'=>'appointments','files'=>true,'class'=>'form-horizontal form-row-seperated']) !!}
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							{!! Form::select('patient_id',
								App\Models\Patient::selectRaw('id as id')
            ->selectRaw('CONCAT("' . trans('admin.name') . ': ",first_name,"  ",father_name,"  ",grand_name,"  - ' . trans('admin.civil') . ': ",civil)  as text')
							->where('id',old('patient_id'))->pluck('text','id')
							,old('patient_id'),['class'=>'form-control get_patient','placeholder'=>trans('admin.patient_id')]) !!}
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							{!! Form::select('attend_status',['pending'=>trans('admin.pending'),'confirmed'=>trans('admin.confirmed'),'unattended'=>trans('admin.unattended'),'attended'=>trans('admin.attended'),],old('attend_status'),['class'=>'form-control','placeholder'=>trans('admin.attend_status')]) !!}
						</div>
					</div>
					<br>
