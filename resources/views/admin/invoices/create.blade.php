...
$(document).on('change','.dr_group_id',function(){
 ...
 @if(old('dr_id') and  old('dr_group_id'))
	$.ajax({
		url:'{{ aurl('load/users/doctor') }}',
		type:'post',
		dataType:'html',
		data:{group_id:'{{ old('dr_group_id') }}',_token:'{{ csrf_token() }}',select:'{{ old('dr_id') }}'},
		success: function(data)
		{
			$('.dr_id').removeClass('hidden');
			$('.dr_id').html(data);
...
$(document).on('change','.accountant_group_id',function(){
 	...
 	{
 		$.ajax({
 			url:'{{ aurl('load/users/accountant') }}',
 			type:'post',
 			dataType:'html',
 			data:{group_id:group_id,_token:'{{ csrf_token() }}'},
 			success: function(data)
 ...
 @if(old('accountant_id') and  old('accountant_group_id'))
	$.ajax({
		url:'{{ aurl('load/users/accountant') }}',
		type:'post',
		dataType:'html',
		data:{group_id:'{{ old('accountant_group_id') }}',_token:'{{ csrf_token() }}',select:'{{ old('accountant_id') }}'},
		success: function(data)
		{
			$('.accountant_id').removeClass('hidden');
			$('.accountant_id').html(data);
...
autosize($('.content'));
	...
	$(add_button).click(function(e){ //on add input button click
		...
		$(wrapper).append('<tr>'+
						'<td>{!! Form::text('price_list[]',old('price_list'),['class'=>'form-control','placeholder'=>trans('admin.price_list')]) !!}</td>'+
						'<td>'+
		...
		<div class="widget-extra body-req portlet light bordered">
			...
			<div class="portlet-body form">
				<div class="col-md-12">
					...
					{!! Form::open(['url'=>aurl('/invoices'),'id'=>'invoices','files'=>true,'class'=>'form-horizontal form-row-seperated']) !!}
            ...
            ->selectRaw('CONCAT("' . trans('admin.name') . ': ",first_name,"  ",father_name,"  ",grand_name,"  - ' . trans('admin.civil') . ': ",civil)  as text')
					...
					<div class="form-group col-md-6">
						...
						<div class="col-md-9">
							{!! Form::select('dr_group_id',App\Models\Group::pluck('group_name','id'),old('dr_group_id'),['class'=>'form-control dr_group_id','placeholder'=>trans('admin.dr_group_id')]) !!}
						</div>
					</div>
					...
					<div class="form-group col-md-6">
						...
						<div class="col-md-9">
							{!! Form::select('accountant_group_id',App\Models\Group::pluck('group_name','id'),old('accountant_group_id'),['class'=>'form-control accountant_group_id','placeholder'=>trans('admin.accountant_group_id')]) !!}
						</div>
					</div>
					<div class="form-group col-md-6">
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							{!! Form::text('invoice_date',old('invoice_date')?old('invoice_date'):date('Y-m-d'),['class'=>'form-control date-picker','placeholder'=>trans('admin.invoice_date'),'readonly'=>'readonly','data-date'=>date("Y-m-d"),'data-date-format'=>'yyyy-mm-dd']) !!}
						</div>
					</div>
					<br>
					<div class="form-group">
...
@if(count(old('content')) > 0)
							...
							<tr>
								<td>{!! Form::text('price_list[]',@old('price_list')[$i],['class'=>'form-control','placeholder'=>trans('admin.price_list')]) !!}</td>
								<td>
...
@endif
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							{!! Form::select('invoice_status',['paid'=>trans('admin.paid'),'unpaid'=>trans('admin.unpaid'),],old('invoice_status'),['class'=>'form-control','placeholder'=>trans('admin.invoice_status')]) !!}
						</div>
					</div>
					<br>
					<div class="form-group">
					...
					<div class="col-md-9">
						{!! Form::select('pay_at',['visa'=>trans('admin.visa'),'cash'=>trans('admin.cash'),],old('pay_at'),['class'=>'form-control','placeholder'=>trans('admin.pay_at')]) !!}
							</div>
					</div>
					<br>
