...
<div class="tab">
  <button class="tablinks1" id="add_count" onclick="openCity2(event, 'mawjoodon')">{{trans('admin.Available')}}
  </button>

  <button class="tablinks1" onclick="openCity2(event, 'mawaeed')">{{trans("admin.Today's dates")}}
    <span>{{$appoints_waiting->count()}}</span> </button>
...
<div id="mawjoodon" class="tabcontent1">
 ...
 foreach ($appoints as $item) { ?>
  <li>
    <a onclick="get_detail('<?=$item->patient_id?>','<?=$item->id?>')"> {{get_status($item->appoint_status)}} :
