...
<td>
    ...
    <input type="hidden" id="p_discount_amount" name="p_discount_amount[]"
        value="{{($product->p_price / 100 * $product->totalDiscountAmount())}}">
...
<td>
    <button type="button" class="btn btn-danger btm-xs deleteButton"><span class="fa fa-trash"></span></button>
</td>
...
<script>
    $(document).ready(function () {
        ...
        function multInputs() {
            ...
            var $total = 0;
            var natio = $("#natio").val();
            var cash_hand = parseFloat($("#cash_hand").val());
            var cash_card = parseFloat($("#cash_card").val());
            if(cash_hand = ''){
                cash_hand =0;
            }
            if(cash_card = ''){
            ...
            $("tr.txt1").each(function () {
                var $qty_price = parseFloat($('.p_price', this).val());
                mult += $qty_price;
            ...
            if (natio!=1 && parseInt('{{setting()->tax_enabled}}')){
                ...
                $("#t_total").html( (mult + tax_amount).toFixed(2));
                $("#t_tax_amount").html(tax_amount.toFixed(2));
            }else{
                $("#t_total").html(mult.toFixed(2));
            }

            $("#due").html(((mult + tax_amount) - cash_hand - cash_card).toFixed(2));
            $("#t_discount").html(t_discount.toFixed(2));
        }
    $(function () {
