  ...
  ">
        <form class="">
            ...
            <span style="display:inline-block;min-width:150px ">
                <input style="border: 1px solid #ddd; border-radius: 5px; padding: 2px 10px;" type="date"
                    value="{{request()->date_from}}" name="date_from"> </span>
            <span>{{__('app.to')}}</span>
            <span style="display:inline-block;min-width:150px ">
                <input type="date" value="{{request()->date_to}}"
                    style="border: 1px solid #ddd; border-radius: 5px; padding: 2px 10px;" name="date_to">
            </span>
            <span>
                <button class="btn btn-warning" type="submit" name="button">{{__('app.show')}} <i
                        class="fas fa-eye"></i></button>
            </span>
        </form>
        <div class="">
            <a href="{{route('report_print')}}" class=" btn btn-success">
                {{__('app.print')}} <i class="fas fa-print"></i>
            </a>
            <a href="{{route('export_report')}}" class=" btn btn-success">
        ...
        <table class="table table-bordered text-center">
            ...
            <tbody>
                <tr>
                    ...
                    <td style="padding:10px  background-color:white; background-color:white;">
                        {{$invoices->sum('paid_cash')}}</td>
                </tr>
                <tr>
                    <td style="width:200px;background-color:#8893B0;color:#fff">{{__('app.network')}}</td>
                    <td style="padding:10px  background-color:white; background-color:white;">
                        {{$invoices->sum('paid_card')}}</td>
                </tr>
                <tr>
                    <td style="width:200px;background-color:#8893B0;color:#fff">{{__('app.okay')}}</td>
                    <td style="padding:10px  background-color:white; background-color:white;">{{$invoices->sum('due')}}
                    </td>
                </tr>
                <tr>
                    <td style="width:200px;background-color:#8893B0;color:#fff"> {{__('app.without_tax')}}</td>
                    <td style="padding:10px  background-color:white; background-color:white;">{{$invoices->sum(function
                        ($item){ return ($item->tax_amount ? ($item->total_amount - $item->tax_amount) :
                        $item->total_amount); })}}</td>
                </tr>
                <tr>
                    <td style="width:200px;background-color:#8893B0;color:#fff">{{__('app.total_with_tax')}}</td>
                    <td style="padding:10px  background-color:white;  background-color:white;">
                        {{$invoices->sum('total_amount')}}</td>
                </tr>
                <tr>
                    <td style="width:200px;background-color:#8893B0;color:#fff">{{__('app.total_tax')}}</td>
                    <td style="padding:10px  background-color:white;  background-color:white;">
                        {{$invoices->sum('tax_amount')}}</td>
                </tr>
            </tbody>
        </table>
    </div>
